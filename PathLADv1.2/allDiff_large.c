...
static void addToDelete(int u, int* list, int* nb, int* marked){
...
static bool updateMatching(int sizeOfU, int sizeOfV, int* degree, int* firstAdj, int*  adj, int* matchedWithU){
	...
	int* matchedWithV = malloc(sizeOfV * sizeof(int));
    int* nbPred = malloc(sizeOfV * sizeof(int));
    int** pred = malloc(sizeOfV * sizeof(int*));
    for (int i = 0; i < sizeOfV; ++i) {
        pred[i] = malloc(sizeOfU * sizeof(int));
        if(pred[i] == NULL) printf("failed\n");
    }
	int *nbSucc = malloc(sizeOfU * sizeof(int));
	int** succ = malloc(sizeOfU * sizeof(int*));
    for (int i = 0; i < sizeOfU; ++i) {
        succ[i] = malloc(sizeOfV * sizeof(int));
    }
    
    int *listV = malloc(sizeOfV * sizeof(int));
	if(listV == NULL) printf("falied\n");
	int *listU = malloc(sizeOfU * sizeof(int));
	
	int *listDV = malloc(sizeOfV * sizeof(int));
	int *listDU = malloc(sizeOfU * sizeof(int));
	
	int *markedV = malloc(sizeOfV * sizeof(int));
	int *markedU = malloc(sizeOfU * sizeof(int));
...
//	int markedV[sizeOfV], markedU[sizeOfU];
    ...
    int *unmatched = malloc(sizeOfU * sizeof(int));
    int *posInUnmatched = malloc(sizeOfU * sizeof(int));
...
static void DFS(int nbU, int nbV, int u, bool* marked, int* nbSucc, int **succ, int* matchedWithU, int* order, int* nb){
...
static void SCC(int nbU, int nbV, int* numV, int* numU, 
		 int* nbSucc, int **succ, 
	...
	//int order[nbU];
	int* order = malloc(nbU * sizeof(int));
   	
	//bool marked[nbU];
	bool *marked = malloc(nbU * sizeof(bool));
	//int fifo[nbV];
...
static bool ensureGACallDiff(bool induced, Tgraph* Gp, Tgraph* Gt, Tdomain* D){
	...
	int *nbPred = malloc(Gp->nbVertices * sizeof(int));
	int ** pred = malloc(Gp->nbVertices * sizeof(int*));
	for(int i = 0;i < Gp->nbVertices;i++) {
	    pred[i] = malloc(Gt->nbVertices * sizeof(int));
	}
	
//	int pred[Gp->nbVertices][Gt->nbVertices]; // pred[u][i] = ith predecessor of u in Go
    
    int *nbSucc = malloc(Gt->nbVertices * sizeof(int));
//	int nbSucc[Gt->nbVertices]; // nbSucc[v] = nb of successors of v in Go
	...
	int **succ = (int**)malloc(Gt->nbVertices * sizeof(int*));
    for(i = 0; i < Gt->nbVertices; i++)
        succ[i] = (int*)malloc(Gp->nbVertices * sizeof(int));

    bool **used = (bool**)malloc(Gp->nbVertices * sizeof(bool*));
    for(i = 0; i < Gp->nbVertices; i++)
        used[i] = (bool*)malloc(Gt->nbVertices * sizeof(bool));

    int *numV = (int*)malloc(Gt->nbVertices * sizeof(int));
    int *numU = (int*)malloc(Gp->nbVertices * sizeof(int));
    int *toMatch = (int*)malloc(Gp->nbVertices * sizeof(int));
...
//	int list[Gt->nbVertices];
	int *list = malloc(Gt->nbVertices * sizeof(int));
	int nb=0;
	...
	return matchVertices_large(nbToMatch,toMatch,induced,D,Gp,Gt);
}

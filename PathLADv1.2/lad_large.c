...
static bool checkLAD(bool induced, int u, int v, Tdomain* D, Tgraph* Gp, Tgraph* Gt){
	...
	if (Gp->nbAdj[u]==1){
		...
		for (i=D->firstVal[u2]; i<D->firstVal[u2]+D->nbVal[u2]; i++){
		  if (
			   (isCompatible(induced, Gp->edgeDirection[u][u2], searchs_lad(Gt,v,D->val[i])))){
	...
	for (i=0; i<Gp->nbAdj[u]; i++){
		...
		if (D->nbVal[u2]>Gt->nbAdj[v]){
			for (j=D->firstVal[u2]; j<D->firstVal[u2]+D->nbVal[u2]; j++){
				...
				if (
					(isCompatible(induced, Gp->edgeDirection[u][u2], searchs_lad(Gt,v,v2)))){ 

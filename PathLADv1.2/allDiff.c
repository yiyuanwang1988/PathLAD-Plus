...
bool ensureGACallDiff(bool induced, Tgraph* Gp, Tgraph* Gt, Tdomain* D){
	...
	int *nbPred = (int *)malloc(sizeof(int) * Gp->nbVertices);
	int pred[Gp->nbVertices][Gt->nbVertices];

	int *nbSucc = (int *)malloc(sizeof(int) * Gt->nbVertices);

	int succ[Gt->nbVertices][Gp->nbVertices];

	int *numV = (int *)malloc(sizeof(int) * Gt->nbVertices);
	int *numU = (int *)malloc(sizeof(int) * Gp->nbVertices);
	int *toMatch = (int *)malloc(sizeof(int) * Gp->nbVertices);
	
	int **used = (int **)malloc(sizeof(int*) * Gp->nbVertices);
	for (ca = 0 ; ca < Gp->nbVertices ; ca ++)
	{
		used[ca] = (int*)malloc(sizeof(int) * Gt->nbVertices);
	}
	...
	for (u=0; u<Gp->nbVertices; u++){
		...
		for (i=0; i<D->nbVal[u]; i++){
			...
			if (v != D->globalMatchingP[u]){
				// printf("u = %d , v = %d , nbPred[u] = %d , nbSucc[v] = %d\n",u,v,nbPred[u],nbSucc[v]);
				pred[u][nbPred[u]++]=v;
				// printf("???\n");
				succ[v][nbSucc[v]++]=u;
			}
	...
	for (u=0; u<Gp->nbVertices; u++){
		...
		for (i=0; i<D->nbVal[u]; i++){
			...
			if ((!used[u][v]) && (numV[v]!=numU[u]) && (D->globalMatchingP[u]!=v))
				if (!removeValue(u,v,D,Gp,Gt)) {
					free(nbPred);
					free(nbSucc);
					free(numU);
					free(numV);
					free(toMatch);
					for(int i = 0;i < Gp->nbVertices;i++) free(used[i]);
					free(used);
					return false;
				}
		}

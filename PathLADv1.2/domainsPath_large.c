...
void printDomains_large(Tdomain *d, int n){
	int u, i;
	for (u=0; u<n; u++){
		printf("D[%d] = ",u); 
...
static bool toFilterEmpty(Tdomain* D){
	// return true if there is no more nodes in toFilter 不用再过滤了
	return (D->nextOutToFilter < 0);
}

static void resetToFilter(Tdomain *D, int size){
	// empty to filter and unmark the vertices that are marked to be filtered
	memset(D->markedToFilter,false,size*sizeof(bool));
	D->nextOutToFilter = -1;
}

static int nextToFilter(Tdomain* D, int size){
...
static void addToFilter(int u, Tdomain* D, int size){
...
static bool isInD(int u, int v, Tdomain* D){
	// returns true if v belongs to D(u); false otherwise 大图中v能否匹配小图中点u
	return (D->posInVal[u][v]<D->firstVal[u]+D->nbVal[u]);
}

static bool augmentingPath(int u, Tdomain* D, int nbV){
	...
	//int fifo[nbV];
	int *fifo = malloc(nbV * sizeof(int));
	//int pred[nbV];
	int *pred = malloc(nbV * sizeof(int));
	int nextIn = 0;
	int nextOut = 0;
	int i, v, v2, u2;
	//bool marked[nbV];
	bool *marked = malloc(nbV * sizeof(bool));
...
static bool removeAllValuesButOne(int u, int v, Tdomain* D, Tgraph* Gp, Tgraph* Gt){
...
bool removeAllValuesButOnes_large(int u, int v, Tdomain* D, Tgraph* Gp, Tgraph* Gt){
	// remove all values but v from D(u) and add all successors of u in toFilter
	// return false if an inconsistency is detected wrt to global all diff
	int j, oldPos, newPos;
	// add all successors of u in toFilter
...
static bool removeValue(int u, int v, Tdomain* D, Tgraph* Gp, Tgraph* Gt){
...
static bool removeValues(int u, int v, Tdomain* D, Tgraph* Gp, Tgraph* Gt){
	// remove v from D(u) and add all successors of u in toFilter
	// return false if an inconsistency is detected wrt global all diff
	int j;
...
static bool isCompatible(bool induced, char dirGp, char dirGt){
...
bool matchVertices_large(int nb, int* toBeMatched, bool induced, Tdomain* D, Tgraph* Gp, Tgraph* Gt){
		...
		for (u2=0; u2<Gp->nbVertices; u2++){
			...
			if (u != u2 && D->val[D->firstVal[u2]] != -1){
				...
				if (Gp->edgeDirection[u][u2] != 0){// remove from D[u2] vertices which are not adjacent to v
                    ...
                    if(Gt->nbAdj[v] < D->nbVal[u2]) {
						int new_size = 0;
						for(int j = 0;j < Gt->nbAdj[v];j++){
							int V = Gt->adj[v][j];
							if(isInD(u2,V,D)){
								int oldPos = D->posInVal[u2][V];
								int newPos = D->firstVal[u2] + new_size;
								D->val[oldPos] = D->val[newPos];
								D->val[newPos] = V;
								D->posInVal[u2][D->val[oldPos]] = oldPos;
								D->posInVal[u2][D->val[newPos]] = newPos;
								new_size++;
							} 
						}
						D->nbVal[u2] = new_size;
                        ...
                        while (j<D->firstVal[u2]+D->nbVal[u2]){
                            ...
                            //printf("%d %d\n",v,D->val[j]);
                            //printf("%d %d %d\n",v,D->val[j],searchs_domain(Gt,v,D->val[j]));
                            if (
                                (isCompatible(induced, Gp->edgeDirection[u][u2], searchs_domain(Gt,v,D->val[j])))) j++;
					...
					j=D->firstVal[u2]; 
					while (j<D->firstVal[u2]+D->nbVal[u2]){
						if (searchs_domain(Gt,v,D->val[j]) == 0) j++;
						else if (!removeValue( u2,D->val[j],D,Gp,Gt)) return false;
					}
				}
...
bool matchVertex_large(int u, bool induced, Tdomain* D, Tgraph* Gp, Tgraph *Gt){
	...
	int *toBeMatched = malloc(Gp->nbVertices * sizeof(int));
	toBeMatched[0]=u;
	if(matchVertices_large(1,toBeMatched,induced,D,Gp,Gt)){
		free(toBeMatched);
		return true;
	}
	free(toBeMatched);
...
Tdomain* createDomains_large(Tgraph* Gp, Tgraph* Gt){
...
bool initDomains_large(bool induced, Tdomain* D, Tgraph* Gp, Tgraph* Gt){
	...
	//int val[Gp->nbVertices*Gt->nbVertices]; // 域大小n1 * n2
	int *val = malloc(sizeof(int) * (Gp->nbVertices*Gt->nbVertices));
	int matchingSize, u, v, valSize;
	int *mu = malloc(sizeof(int) * (Gp->maxDegree+1));
    //int mu[Gp->maxDegree+1];
    
    int **mv = malloc(sizeof(int*) * Gt->nbVertices);
    for(int i = 0;i < Gt->nbVertices;i++) 
        mv[i] = malloc(sizeof(int) * (Gp->maxDegree+1));
   ...
   // memset(mv,0,(Gp->maxDegree+1)*Gt->nbVertices*sizeof(int));
	for(int i = 0;i < Gt->nbVertices;i++) {
   		 memset(mv[i], 0, (Gp->maxDegree+1)*sizeof(int));
	}
	
    for (int v=0; v<Gt->nbVertices; v++){ // 大图中所有点 : v
        for (int i=0; i<Gt->nbAdj[v]; i++){ // 大图中v的邻居 : Gt->Adj[v][i]
            if (Gt->nbAdj[Gt->adj[v][i]] < Gp->maxDegree) // v的邻居的度数小于小图中最大度数
                mv[v][Gt->nbAdj[Gt->adj[v][i]]]++; // v的邻居中该点的度数 + 1
            else
                mv[v][Gp->maxDegree]++; // v的邻居中度数 >= maxDegree的点数
        }
    }
